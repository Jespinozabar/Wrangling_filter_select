...
The code doesn’t work because it tries to filter on the year column after that column has been removed by select(), so year no longer exists when filtering is applied.
...
movies %>%
  ...
  slice(1)
```
Cure for insomnia was the longest movie. 
...
```
The oldest movie is Blacksmith scene.
...
1248 movies were realeased in 1995.
...
These are the Comedy movies (including mixed genres like "Comedy") that have an MPAA rating, run longer than 120 minutes, and have an IMDb rating above 8, making them well-rated, long comedies with official content ratings.
...
```{r}
# Subset of Harry Potter movies
hp_movies <- movies %>%
  filter(str_detect(title, "Harry Potter")) %>%
  select(title, rating, budget, votes)

# View all Harry Potter movies included
hp_movies

# Highest rated Harry Potter movie
hp_movies %>%
  arrange(desc(rating)) %>%
  slice(1)

# Harry Potter movie with the most votes
hp_movies %>%
  arrange(desc(votes)) %>%
  slice(1)
...
The subset includes all movies with "Harry Potter" in the title, showing their title, rating, budget, and votes. The highest-rated Harry Potter movie is Harry Potter and the Deathly Hallows: Part 2, and the one with the most IMDb votes is Harry Potter and the Sorcerer's Stone.
...
this is the movie with the highest budget among all movies that are either Animation or Comedy( wild wild west).
...
# Check if favorite movie is in the dataset
if (my_favorite %in% movies$title) {
  print(paste(my_favorite, "is in the dataset!"))
} else {
  print(paste(my_favorite, "is NOT in the dataset."))
}

# Filter movies with rating > 7, select title and year, arrange by rating desc
filtered_sorted <- movies %>%
  filter(rating > 7) %>%
  select(title, year, rating) %>%
  arrange(desc(rating))

# Show top 10 filtered and sorted movies
print(head(filtered_sorted, 10))

```

Tangled is my favorite movie and it was in the dataset, then my code displayed the top 10 highest-rated movies with a rating over 7, and also shows other questions you can explore using filter(), select(), and arrange().
